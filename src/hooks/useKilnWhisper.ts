...
export function useKilnWhisper() {
  ...
  const switchLoad = useCallback((id: string) => {
    ...
    setSelected(new Set())
    setAnnouncement('Bake batch changed and filters reset.')
  }, [])
  ...
  const markSelected = useCallback((finished: boolean) => {
    setLoads((current) => current.map((load) => load.id !== activeLoad?.id ? load : ({ ...load, pieces: load.pieces.map((piece) => selected.has(piece.id) ? { ...piece, finished } : piece) })))
    setAnnouncement(selected.size + ' selected bake' + (selected.size === 1 ? '' : 's') + ' marked ' + (finished ? 'finished' : 'waiting') + '.')
    setSelected(new Set())
  ...
  const addPiece = useCallback((piece: Omit<Piece, 'id' | 'number'>) => {
    ...
    setAnnouncement(piece.name + ' added to the bake batch.')
  }, [activeLoad?.id])
...
export type CrumbLedgerState = ReturnType<typeof useKilnWhisper>
